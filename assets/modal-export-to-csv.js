...
class ModalExportToCsv {
    ...
    constructor() {
        this.#createExportToCsvBatchActions()
    }

    #createExportToCsvBatchActions() {


        const modalTitle = document.querySelector('#batch-action-confirmation-title-csv');
        ...
        document.querySelectorAll('[data-action-batch]').forEach((dataActionBatch) => {
            dataActionBatch.addEventListener('click', (event) => {
                ...
                document.querySelector('#modal-batch-action-button-csv').addEventListener('click', () => {
