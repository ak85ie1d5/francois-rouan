...
final class TableConfigurator implements FieldConfiguratorInterface
{
    ...
    {
        return TableField::class === $field->getFieldFqcn();
    }

    public function configure(FieldDto $field, EntityDto $entityDto, AdminContext $context): void
    {
        if (null !== $entryTypeFqcn = $field->getCustomOptions()->get(TableField::OPTION_ENTRY_TYPE)) {
        ...
        // the contents of this field are a table of other fields, so it cannot be sorted
        $field->setSortable(false);

        $field->setFormTypeOptionIfNotSet('allow_add', $field->getCustomOptions()->get(TableField::OPTION_ALLOW_ADD));
        $field->setFormTypeOptionIfNotSet('allow_delete', $field->getCustomOptions()->get(TableField::OPTION_ALLOW_DELETE));
        ...
        $field->setFormTypeOptionIfNotSet('entry_options.label', $field->getCustomOptions()->get(TableField::OPTION_SHOW_ENTRY_LABEL));

        // table items range from a simple <input text> to a complex multi-field form
        // the 'entryIsComplex' setting tells if the table item is so complex that needs a special
        // rendering not applied to simple table items
        if (null === $field->getCustomOption(TableField::OPTION_ENTRY_IS_COMPLEX)) {
            $definesEntryType = null !== $entryTypeFqcn = $field->getCustomOption(TableField::OPTION_ENTRY_TYPE);
        ...
        if (true === $field->getCustomOption(TableField::OPTION_ENTRY_USES_CRUD_FORM)) {
            ...
            $crudNewPageName = $field->getCustomOption(TableField::OPTION_ENTRY_CRUD_NEW_PAGE_NAME) ?? Crud::PAGE_NEW;
            ...
            try {
                ...
                throw new \RuntimeException(sprintf('The "%s" table field of "%s" uses the "useEntryCrudForm()" method, which requires Symfony 6.1 or newer to work. Upgrade your Symfony version or use instead the "setEntryType()" method to render the collection entries using a Symfony form.', $field->getProperty(), $context->getCrud()?->getControllerFqcn()), 0, $exception);
            }
        }
    }
